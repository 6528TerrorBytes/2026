...
public class ArmMove extends Command {
  ...
  private final boolean m_oscillating;

  /** Creates a new ArmMove. */
  public ArmMove(IntakeArm arm, double setPoint, boolean oscillating) {
   m_arm = arm;
   m_setPoint = setPoint;
  ...
  public void execute() {
    if (m_oscillating) {
      if (m_arm.atGoal()) {
        if (m_setPoint == Constants.Setpoints.armOscillateUp) {
          new ArmMove(m_arm, Constants.Setpoints.armOscillateDown, true);
        } else if (m_setPoint == Constants.Setpoints.armOscillateDown) {
          new ArmMove(m_arm, Constants.Setpoints.armOscillateUp, true);
        }
      }
    }
    // if (!m_hasMoved) {
